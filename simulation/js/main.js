...
// Update visibility of content based on aperture type
function updateContentVisibility() {
  console.log("updateContentVisibility called. ApertureType:", apertureType);
  const singleSlitContent = document.querySelectorAll('.content-single-slit');
  const circularContent = document.querySelectorAll('.content-circular');
  const slitWidthLabel = document.getElementById('ip2a');

  if (apertureType === "single-slit") {
    singleSlitContent.forEach(function (el) {
      el.classList.add('content-show');
      console.log("Showing single slit content");
    });
    circularContent.forEach(function (el) {
      el.classList.remove('content-show');
    });
    if (slitWidthLabel) slitWidthLabel.innerHTML = "Width of slit:";
  } else {
    singleSlitContent.forEach(function (el) {
      el.classList.remove('content-show');
    });
    circularContent.forEach(function (el) {
      el.classList.add('content-show');
      console.log("Showing circular aperture content");
    });
    if (slitWidthLabel) slitWidthLabel.innerHTML = "Diameter:";
  }
}
...
function start() {
  ...
  if (apertureDropdown) {
    apertureDropdown.onchange = function () {
      ...
      if (typeof updateContentVisibility === 'function') updateContentVisibility();
      reaction(true);
    };
  }
  ...
  if (typeof updateContentVisibility === 'function') updateContentVisibility(); // Initial visibility update
